...
interface LatestNewsProps {
    viewMode?: 'headline' | 'grid'
}

export default function LatestNews({ viewMode = 'headline' }: LatestNewsProps) {
        ...
        return (
            <div className="container mx-auto px-4 md:px-20">
                <div className="mb-12 border-b border-slate-200 pb-6 flex justify-between items-end">
                    <div>
                        <h1 className="font-bold text-[#2A3955] text-4xl mb-2">Kumpulan Berita</h1>
                        <p className="text-slate-500 text-sm">Temukan informasi dan pengumuman terbaru dari kampus UIB.</p>
                    </div>
                    <div className="text-xs text-slate-400 font-medium">
                        {allNews.length} Total Berita
                    </div>
                </div>
                    ...
                    {allNews.map((news) => (
                        ...
                        >
                            ...
                            <div className="p-4 flex flex-col flex-1">
                                ...
                                <h4 className="text-[15px] font-bold text-[#2A3955] leading-snug line-clamp-2 mb-3 group-hover:text-[#0055aa] transition-colors">
                                    {news.title}
                                </h4>
                                <div className="mt-auto pt-3 border-t border-slate-50 flex items-center justify-between">
                                    <span className="text-[10px] text-slate-400 italic">By {news.author}</span>
                                    <span className="text-[11px] font-bold text-[#e67e22] flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        Baca <FaArrowRight size={8} />
                                    </span>
                                </div>
                            </div>
                        </Link>
                ...
                {allNews.length === 0 && (
                    <div className="py-20 text-center text-slate-400 italic bg-white rounded-2xl border-2 border-dashed border-slate-100">
    ...
    return (
        <section className={`py-12 bg-[#F7F7F7] font-poppins text-[#2A3955] ${viewMode === 'grid' ? 'min-h-[60vh]' : ''}`}>
            {viewMode === 'headline' ? renderHeadlineView() : renderGridView()}
        </section>
    );
}
