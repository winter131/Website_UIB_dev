...
export default function CreatePrestasiForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch('/api/prestasi', { method: 'POST', body: formPayload });
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || 'Terjadi kesalahan');
            }
            alert('Prestasi berhasil dipublikasikan!');
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <input type="text" name="source" value={formData.source} onChange={handleChange} placeholder="UIB.AC.ID"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <input type="text" name="author" required value={formData.author} onChange={handleChange} placeholder="Humas UIB"
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all" />
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <textarea name="summary" required value={formData.summary} onChange={handleChange} rows={2}
                    ...
                    placeholder="Penjelasan singkat untuk kartu prestasi..." />
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 transition-all bg-gray-50/30">
                    <NotionEditor
                        ...
                        placeholder="Mulai menulis detail prestasi... Gunakan '/' untuk perintah cepat."
                    />
                </div>
            ...
            <div className="flex items-center justify-end gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="submit" disabled={isSubmitting}
                    ...
                    {isSubmitting ? 'Mempublikasikan...' : 'Publikasikan Prestasi'}
                </button>
            </div>
        </form>
    );
