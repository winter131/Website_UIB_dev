...
import { useCalonMahasiswaBelumDaftarUlangData } from "@/hooks/calon-mahasiswa/useCalonMahasiswaBelumDaftarUlangData";
...
export default function PerubahanGelombangView() {
  ...
  const { data, isLoading, refetch } = useCalonMahasiswaBelumDaftarUlangData(
  ...
  const { mutate: updatePerubahanGelombangMutation } =
    useEditPerubahanGelombang(
      () => {
        showNotification({
          status: "text-green-500",
          icon: "bx bx-check text-2xl",
          header: "Berhasil",
          message: "Berhasil mengedit perubahan gelombang calon mahasiswa",
        });
        refetch();
      },
      (msg) => {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("appliedFilterPerubahanGelombang");
    const search = localStorage.getItem("appliedSearchPerubahanGelombang");
    if (saved) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "appliedFilterPerubahanGelombang",
      JSON.stringify({
    ...
    localStorage.setItem("appliedSearchPerubahanGelombang", searchTerm);
  ...
  const handleUbahGelombang = (
    data: CalonMahasiswaType,
    selectedGelombang: DetailGelombangType,
  ) => {
    showConfirmation({
      title: "Ubah Gelombang Calon Mahasiswa?",
      message: `Ubah gelombang pendaftaran calon mahasiswa ${data.NamaCamhs} - (${data.NomorDaftar}) menjadi ${selectedGelombang.NamaGelombang} - ${ucFirst(selectedGelombang.NamaPeriode)}?`,
      icon: "edit-alt",
      confirmButtonText: "Ubah",
      confirmButtonColor: "bg-green-500",
      onConfirm() {
        updatePerubahanGelombangMutation({
          token: session.user?.accessToken,
          perubahanGelombang: {
            sel_camhs: data.NomorDaftar,
            gelombang_baru: Number(selectedGelombang.detail_gelombang_id),
          },
        });
      },
    });
    ...
    <div className="px-8 py-4">
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Perubahan Gelombang</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Modul untuk manajemen perubahan gelombang calon mahasiswa. Mahasiswa
          yang tampil pada halaman ini adalah mahasiswa yang belum melakukan
          daftar ulang.
        </span>
      </div>
      ...
      <div className="w-full">
        ...
        <DataTable
          ...
          isLoading={isLoading}
          refetch={refetch}
          handleUbahGelombang={handleUbahGelombang}
