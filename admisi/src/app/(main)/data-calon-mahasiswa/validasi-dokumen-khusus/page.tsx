...
import ValidasiDokumenKhususView from "./view/ValidasiDokumenKhususView";

export default function ValidasiDokumenKhusus() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <ValidasiDokumenKhususView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
