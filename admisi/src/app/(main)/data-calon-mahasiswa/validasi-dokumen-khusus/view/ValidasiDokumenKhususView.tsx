...
export default function ValidasiDokumenKhususView() {
    ...
    if (filter.keteranganDokumen === "" || filter.keteranganDokumen === "all") {
      ...
      const filteredData = (data.all_camhs || []).filter(
        (camhs: CalonMahasiswaType) => {
          const status = camhs.StatusDokumen;
  ...
  useEffect(() => {
    const saved = localStorage.getItem("filterValidasiDokumenKepalaHumas");
    const search = localStorage.getItem("searchTermValidasiDokumenKepalaHumas");
    if (saved) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "filterValidasiDokumenKepalaHumas",
      JSON.stringify({
    ...
    localStorage.setItem("searchTermValidasiDokumenKepalaHumas", searchTerm);
    ...
    <div className="px-8 py-4">
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Validasi Dokumen Khusus</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Modul untuk manajemen validasi dokumen pendaftaran calon mahasiswa
        </span>
      </div>
