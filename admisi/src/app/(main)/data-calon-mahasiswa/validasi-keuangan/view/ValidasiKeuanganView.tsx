...
export default function ValidasiKeuanganView() {
    ...
    if (filter.statusPembayaran === "" || filter.statusPembayaran === "all") {
      ...
      const filteredData = (data.all_camhs || []).filter(
        (camhs: CalonMahasiswaType) => {
          const status = camhs.StatusKeuangan;

          if (filter.statusPembayaran === "lunas") {
            return status === "sudah";
          }

          if (filter.statusPembayaran === "menunggu") {
            return status === "belum" && !!camhs.LinkBuktiBayar;
          }

          if (filter.statusPembayaran === "belum_upload") {
            return status === "belum" && !camhs.LinkBuktiBayar;
          }

          if (filter.statusPembayaran === "tolak") {
            return status === "tolak";
          }
  ...
  return (
    <div className="px-8 py-4">
      <h1 className="w-full text-4xl font-bold text-black">
        Validasi Keuangan Calon Mahasiswa
      </h1>
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Validasi Keuangan Calon Mahasiswa</Link>
          </li>
        </ul>
      </div>
      ...
      {/* <KeuanganFilter onFilterChange={handleFilterChange} /> */}
