...
import HapusImportDaftarUlangView from "./view/HapusImportDaftarUlangView";

export default function HapusImportDaftarUlang() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <HapusImportDaftarUlangView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
