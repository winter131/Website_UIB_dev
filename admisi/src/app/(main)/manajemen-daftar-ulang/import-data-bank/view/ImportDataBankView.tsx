...
import { useGetImportBankDataDetail } from "@/hooks/import-data-bank/useGetImportBankDataDetail";
...
export default function ImportDataBankView() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("appliedFilterImportDataBank");
    const search = localStorage.getItem("appliedSearchImportDataBank");
    if (saved) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      "appliedFilterImportDataBank",
      JSON.stringify({
        selBulan: String(filter.selBulan),
        selTahun: String(filter.selTahun),
      }),
    );
    localStorage.setItem("appliedSearchImportDataBank", searchTerm);
  ...
  return (
    <div className="px-8 py-4">
      ...
      <h1 className="w-full text-4xl font-bold text-black">Import Data Bank</h1>
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Import Data Bank</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Modul untuk manajemen import data pembayaran calon mahasiswa
        </span>
        <hr className="my-4" />
      </div>
