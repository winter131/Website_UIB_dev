...
import TagihanLOAMagisterView from "./view/TagihanLOAMagisterView";

export default function TagihanLOAMagister() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <TagihanLOAMagisterView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
