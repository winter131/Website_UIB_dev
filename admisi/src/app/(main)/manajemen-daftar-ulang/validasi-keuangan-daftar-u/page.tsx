...
import ValidasiKeuanganDaftarUlangView from "./view/ValidasiKeuanganDaftarUlangView";

export default function ValidasiKeuanganDaftarUlang() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <ValidasiKeuanganDaftarUlangView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
