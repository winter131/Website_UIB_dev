...
export default function ValidasiKeuanganDaftarUlangView() {
    ...
    if (filter.statusDaftarUlang === "" || filter.statusDaftarUlang === "all") {
      ...
      const filteredData = (data.all_camhs || []).filter(
        (camhs: CalonMahasiswaType) => {
          const status = camhs.StatusKeuanganDaftarUlang;
          ...
          if (filter.statusDaftarUlang === "tolak") {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(
      "appliedFilterValidasiKeuanganDaftarUlang",
    );
    const search = localStorage.getItem(
      "appliedSearchValidasiKeuanganDaftarUlang",
    );
    if (saved) {
    ...
    <div className="px-8 py-4">
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Validasi Keuangan Daftar Ulang</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Modul untuk manajemen validasi keuangan daftar ulang calon mahasiswa
        </span>
      </div>
