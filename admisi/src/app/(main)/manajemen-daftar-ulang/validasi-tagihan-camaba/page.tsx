...
import ValidasiTagihanCamabaView from "./view/ValidasiTagihanCamabaView";

export default function ValidasiTagihanCamaba() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <ValidasiTagihanCamabaView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
