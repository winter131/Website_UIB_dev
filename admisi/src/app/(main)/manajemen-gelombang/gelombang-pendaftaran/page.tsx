...
import GelombangPendaftaranView from "./view/GelombangPendaftaranView";

export default function LokasiUjian() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <GelombangPendaftaranView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
