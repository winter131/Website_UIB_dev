...
import { useUbahWawancara } from "@/hooks/hasil-wawancara/useUbahWawancara";

export default function UbahWawancaraView({
  nomorDaftar,
  namaCamhs,
}: {
  nomorDaftar: string;
  ...
  // Ubah wawancara mutation
  const { mutate: ubahWawancaraMutation } = useUbahWawancara(
    () => {
      showNotification({
        status: "text-green-500",
        icon: "bx bx-check text-2xl",
        header: "Berhasil",
        message: "Berhasil mengubah data wawancara calon mahasiswa",
      });
      router.replace("/manajemen-usm/hasil-wawancara");
    },
    (msg) => {
  ...
  return (
    <div className="px-8 py-4">
      <h1 className="w-full text-4xl font-bold text-black">
        Ubah Hasil Wawancara - {namaCamhs}
      </h1>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Ubah data hasil wawancara calon mahasiswa
        </span>
      </div>
