...
import { useHasilWawancaraData } from "@/hooks/hasil-wawancara/useHasilWawancaraData";
import { useDeleteHasilWawancara } from "@/hooks/hasil-wawancara/useDeleteHasilWawancara";
import { HasilWawancaraType } from "@/types/HasilWawancaraTypes";

export default function HasilWawancaraView() {
  ...
  const { data, isLoading, refetch } = useHasilWawancaraData(
    session.user?.accessToken,
    status,
    filter,
  );

  const { mutate: deleteHasilWawancaraMutation } = useDeleteHasilWawancara(
    () => {
      showNotification({
        status: "text-green-500",
        icon: "bx bx-check text-2xl",
        header: "Berhasil",
        message: "Berhasil menghapus hasil wawancara",
      });
      refetch();
      setSearchTerm("");
    },
  ...
  useEffect(() => {
    const saved = localStorage.getItem("appliedFilterHasilWawancara");
    const search = localStorage.getItem("appliedSearchHasilWawancara");
    if (saved) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("appliedSearchHasilWawancara", searchTerm);
  ...
  // Handle delete
  const handleDelete = (data: HasilWawancaraType) => {
    showConfirmation({
      title: "Hapus Hasil Wawancara?",
      message: `Hasil wawancara ${data.NamaCamhs} - (${data.NomorDaftar})? ini akan dihapus dari sistem dan tidak dapat dikembalikan. Lanjut hapus?`,
      icon: "trash",
      confirmButtonText: "Hapus",
      confirmButtonColor: "bg-red-600",
      onConfirm() {
        deleteHasilWawancaraMutation({
          token: session.user?.accessToken,
          hasilWawancaraId: data.NomorDaftar,
        });
      },
    });
  };
  ...
  const handleDetail = (data: HasilWawancaraType) => {
    router.push(
      `/manajemen-usm/hasil-wawancara/${data.NomorDaftar}/${encodeURIComponent(data.NamaCamhs)}`,
    );
  };

  return (
    <div className="px-8 py-4">
      <h1 className="w-full text-4xl font-bold text-black">Hasil Wawancara</h1>
      ...
      <div className="breadcrumbs text-sm text-black">
        <ul>
          ...
          <li className="text-xs">
            <Link href={pathname}>Hasil Wawancara</Link>
          </li>
        </ul>
      </div>
      ...
      <div className="mt-2 flex flex-wrap gap-4">
        <span className="font-normal text-black">
          Modul untuk manajemen hasil wawancara calon mahasiswa
        </span>
      </div>
