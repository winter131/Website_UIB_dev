...
import ValidasiKeuanganRPLView from "./view/ValidasiKeuanganRPLView";

export default function ValidasiKeuanganRPL() {
  ...
  return !isLoading && isFetched ? (
    <>{permission ? <ValidasiKeuanganRPLView /> : <NotAllowedPage />}</>
  ) : (
    <LoadingPermission />
  );
}
