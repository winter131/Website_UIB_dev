...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await axios.get(`${process.env.SERVICE_URL}/v2/tagihan-camhs`, {
      headers: {
        Authorization: `${authHeader}`,
      },
      params: {
        sel_jenjang: selJenjang,
        sel_periode: selPeriode,
      },
    });
    ...
    if (res.data.status === 200) {
      ...
      return NextResponse.json(
        {
          message:
            res.data.error || "Gagal mengambil data tagihan calon mahasiswa",
        },
        { status: 500 },
      );
    }
