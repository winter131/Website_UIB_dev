...
export async function GET(req: NextRequest) {
  ...
  const tujuanFile = params.get("tujuanFile") || "";

  try {
    const res = await axios.get(
      `${process.env.SERVICE_URL}/v2/calon-mahasiswa/generate-format-excel-va-camhs`,
      {
        headers: {
    ...
    if (res.data.status === 200) {
      ...
      return NextResponse.json(
        { message: res.data.error || "Gagal generate format excel" },
        { status: 500 },
      );
    }
  } catch (error: any) {
