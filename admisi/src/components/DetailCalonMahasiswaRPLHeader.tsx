...
export default function DetailCalonMahasiswaRPLHeader({
  isLoading,
  data,
}: {
  isLoading: boolean;
  data: CalonMahasiswaRPLType | null;
}) {
  const isPhotoPdf =
    data?.DokumenData?.PasPhotoLink &&
    data?.DokumenData?.PasPhotoLink?.toLowerCase()
      ?.split("?")[0]
      ?.endsWith(".pdf");
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col">
          ...
          <div className="text-dark dark:text-white font-light text-sm flex flex-row gap-3 md:flex-row flex-wrap md:gap-3">
            ...
            <span>{ucFirst(data?.BiodataCamhs?.WaktuKuliah || "-")}</span>

            <span className="opacity-60">&#9679;</span>

            <span>{ucFirst(data?.BiodataCamhs?.JalurPendaftaran || "-")}</span>
          </div>
          {isPhotoPdf && (
            <div className="mt-2 text-center flex flex-row items-center gap-2">
              ...
              <Link
                href={data?.DokumenData?.PasPhotoLink || "#"}
