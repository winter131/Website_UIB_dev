import { saveValidasiKeuanganDaftarUlang } from "@/service/calonMahasiswa.service";
import { CalonMahasiswaType } from "@/types/CalonMahasiswaTypes";
import { useMutation } from "@tanstack/react-query";

export function useSaveValidasiKeuanganDaftarUlang(
  onSuccess?: (data: CalonMahasiswaType) => void,
  onError?: (msg: string) => void,
) {
  return useMutation({
    mutationFn: ({ token, validasi }: any) =>
      saveValidasiKeuanganDaftarUlang(token, validasi),
    ...
    onError: (error: any) =>
      onError?.(
        error.response?.data?.message ??
          "Gagal melakukan validasi keuangan daftar ulang",
      ),
  });
}
